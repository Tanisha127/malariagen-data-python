...
class AnophelesFrequencyAnalysis(AnophelesBase):
    ...
    def plot_frequencies_time_series(
        ...
        **kwargs,
        ...
        if not dfs:
            raise ValueError(
                f"No cohorts remain after filtering. "
                f"Please check the values passed to the `taxa` parameter "
                f"({taxa!r}) and the `areas` parameter ({areas!r}), "
                f"and ensure they match cohorts present in the dataset."
            )
